@extends('layouts.app', ['activePage' => 'city', 'navName' => 'City' ,'title' => 'City Update',  
                            ...
                            <div class="form-group col-md-4">
                                    ...
                                    @foreach ($state as $state)
                                        <option value="{{ $state->id }}">
                                            <a class="dropdown-item" name="state_id"
                                                href="{{ route('city.show', $state->id) }}">{{ $state->name }}</a>
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                             ...
                             <div class="form-group">
                                 ...
                                 <input type="text" class="form-control" id="shortName" name="shortName" aria-describedby="emailHelp"
                                     placeholder="Enter ShortName " value="{{ $city->shortName }}">
                             </div>

                             <div class="form-group">
                                 <label for="Description">Description :</label>
                                 <input type="text" class="form-control" id="description" name="description" aria-describedby="emailHelp"
                                     placeholder="Enter Description " value="{{ $city->description }}">
                             </div>

                             <button type="submit" class="btn btn-primary mt-2">Update</button>
                             <a href="{{ route('city.index') }}" class="btn btn-light">Cancel</a>
